...
### 6. Запуск Java-файлов на компьютере и работа с JAR

- Чтобы скомпилировать исходный код в байт-код нужно использовать команду javac -encoding UTF-8 <имя java-файла>

`$ javac -encoding UTF-8 HelloJdk.java`

после будет создан файл <имя java-файла>.class

- чтобы запустить файл  с байт кодом нужно использовать команду java <имя class-файла без расширения>

`$ java -Dfile.encoding=UTF-8 HelloJdk`

- чтобы скомпилировать несколько файлов из указанной директории(src) нужно использовать команду javac -d  *.java

`$ javac -d bin -encoding UTF-8 src/*.java`

или из текущей директории

`$ javac -d bin -encoding UTF-8 HelloJdk.java`

- -d bin Так мы указываем где должен будет создан файл .class с скомпилированный кодом

- чтобы запустить программу -cp <имя папки, в которой лежат class-файлы>

`$ java -cp src Practicum `

### 7. JAR 

- для того чтобы создать JAR нужно использовать команду jar cfe <имя jar-файла> <имя стартового класса> <список файлов>

`$ jar cfe library.jar Practicum -C src .`

- для запуска JAR файла нужна команда java -jar <имя jar-файла>
